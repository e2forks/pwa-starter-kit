...
import { addReducers } from "../store.js";

// We are lazy loading its reducer.
import app from '../reducers/app';
const store = addReducers({ app });
...
// local elements needed by this element
import "./snack-bar";
import { menuIcon } from "./my-icons";

// interface needed by this element
...
class MyApp extends connect(store)(LitElement) {
  ...
  render() {
    ...
    <app-drawer ?opened="${_drawerOpened}"
        @opened-changed="${(e: Event) =>
          store.dispatch(updateDrawerState((e.target as any).opened))}">
  ...
  updated(changedProperties: PropertyValues) {
    if (changedProperties.has("_page")) {
      const { appTitle } = this;
      const pageTitle = `${appTitle} - ${changedProperties.get("_page")}`;
      updateMetadata({
        title: pageTitle,
  ...
  _stateChanged(state: IState) {
    this._page = state.app.view as string;
