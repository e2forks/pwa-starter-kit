...
export interface ICart {
  addedIds: string[];
  quantityById: IObjectMap<number>;
}
...
const shop: Reducer<IShopState, Actions> = (
  state = { products: {}, cart: INITIAL_CART },
  action
) => {
  switch (action.type) {
    case GET_PRODUCTS:
    ...
    case CHECKOUT_SUCCESS:
      return {
        ...state,
        products: products(state.products, action as ProductActions),
...
const products = (state: IProductMap, action: ProductActions) => {
  switch (action.type) {
    case ADD_TO_CART:
    case REMOVE_FROM_CART:
      ...
      return {
        ...state,
        [productId]: product((state as IProductMap)[productId], action)
      };
    default:
      return state;
  }
...
const product = (state: IProduct, action: ProductActions) => {
  switch (action.type) {
    case ADD_TO_CART:
      return {
        ...state,
...
const cart = (state: ICart = INITIAL_CART, action: Actions) => {
  switch (action.type) {
    case ADD_TO_CART:
    case REMOVE_FROM_CART:
      return {
...
const addedIds = (
  state: string[] = INITIAL_CART.addedIds,
  quantityById: IObjectMap<number>,
  ...
  switch (action.type) {
    case ADD_TO_CART:
      if (state.indexOf(productId as string) !== -1) {
        return state;
      }
      return [...state, action.productId as string];
    case REMOVE_FROM_CART:
...
const quantityById = (
  state: IObjectMap<number> = INITIAL_CART.quantityById,
  action: ProductActions
) => {
  const productId = action.productId;
  switch (action.type) {
...
const cartSelector = (state: IState) => state.shop.cart;
const productsSelector = (state: IState) => state.shop.products;
